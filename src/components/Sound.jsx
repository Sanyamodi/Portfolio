...
const Sound = () => {
  ...
  const isPlayingRef = useRef(false); // 👈 tracks latest isPlaying
  const [showModal, setShowModal] = useState(false);

  useEffect(() => {
    isPlayingRef.current = isPlaying; // update ref whenever isPlaying changes
  }, [isPlaying]);

  useEffect(() => {
    const handleFirstUserInteraction = () => {
      const musicConsent = localStorage.getItem("musicConsent");
      if (musicConsent === "true" && !isPlayingRef.current) {
  ...
  }, []); // ✅ safe empty dependency array now
